...
class Program:
	...
	def evaluateParameters(self, params):
		...
		if self.command == "":
			raise ConfigurationException("Command wasn't been configured, execute with /? for more information")

		if self.inputPath == "":
			raise ConfigurationException("InputPath wasn't been configured, execute with /? for more information")

		if self.outputPath == "":
			raise ConfigurationException("OutputPath wasn't been configured, execute with /? for more information")

		if self.xmlPath == "" and self.command == self.paymentFlow:
			raise ConfigurationException("XmlPath wasn't been configured, execute with /? for more information")
	...
	def loadParameters(self):
		...
		for line in lines:
			if (not line.strip() == "") and (not "//" in line[:5]):
				values = line.split("=")

				if len(values) < 2:
					raise ConfigurationException("No value defined in [%s] line of ini file." % line)
	...
	def writeResponse(self, message):
		...
		f.close()
		logInfo("File [%s] has been written." % self.outputPath)
...
class SkdServices:
	...
	def fillDictionary(self, parameters, fields):
		...
		for field in fields:
			try:
				...
				raise FieldNotFoundException("Field %s wasn't been provided in data file " % field)
	...
	def loadPaymentMessage(self, parameters, xmlFile):
		...
		xmlPayload = xmlPayload.replace("${" + self.PublicRequestKey + "}", parameters[self.PublicRequestKey].replace("t", ""))
...
except NoResponseKeyException, exc:
	logError(exc.message)
